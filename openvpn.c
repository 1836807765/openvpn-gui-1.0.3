...
//  strncpy(return_string, " Select_Cert", sizeof(return_string));
 ...
 // 	  strncat(command_line, return_string, sizeof(command_line) - strlen(command_line) - 1);
  //  }
    ...
    //MessageBox(NULL, return_string, "mey-test", MB_OK);
    
    //return_string = "--cryptoapicert \"THUMB:7a 86 ee 18 da 83 da 24 7c 3f 76 81 6d a6 13 6f 2a 37 d3 42\"";
		//MessageBox(NULL, return_string, "mey-test", MB_OK);
		//MessageBox(NULL, o.exe_path, "mey-test", MB_OK);
		//MessageBox(NULL, o.cnn[config].config_dir, "mey-test", MB_OK);		
		//strncat(command_line, return_string, sizeof(command_line) - strlen(command_line) - 1);
...
BOOL CALLBACK StatusDialogFunc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  HWND hwndLogWindow;
  RECT rect;
  RECT rect_dlg;//add by mey 20130904
  ...
  switch (msg) {
    ...
    case WM_INITDIALOG:
      ...
      charformat.yHeight = 200;//note by mey 20131106 此地修改字符大小
      ...
      //add by mey 20130904 start修改log对话框的尺寸
      //GetWindowRect(hwndDlg, &rect_dlg);
      //MoveWindow (hwndDlg, rect_dlg.left, rect_dlg.top, 360, 150, TRUE);
      //add by mey 20130904 end
      GetClientRect(hwndDlg, &rect);
      MoveWindow (hwndLogWindow, 20, 25, rect.right - 40, rect.bottom - 70, TRUE);
      //modify by mey start 修改状态显示的位置，使其显示在中间(已恢复为原有代码)
      MoveWindow (GetDlgItem(hwndDlg, TEXT_STATUS), 20, 5, rect.right - 25, 15, TRUE);
      //MoveWindow (GetDlgItem(hwndDlg, TEXT_STATUS), rect.right/2-60, (rect.bottom-23)/2, rect.right - 25, 15, TRUE);
      //modify by mey end
    ...
    case WM_SIZE:
      ...
      //MoveWindow (GetDlgItem(hwndDlg, TEXT_STATUS), LOWORD (lParam)/2-60, (HIWORD (lParam)-23)/2, LOWORD (lParam) - 25, 15, TRUE);
      //modify by mey end
...
int CheckUSBKeyChanged()
{
  ...
  if(p != NULL)
  	*p=0x00;
  strncat(file_path, "\\m_signe.txt", 12);  
  if (!(fp=fopen(file_path, "r")))
  {
  	MessageBox(NULL, "打开文件失败！", "提示信息", MB_OK);
  	return 0;
