...
GetRegistryKeys()
{
  ...
  if (!GetRegKey("allow_password", o.allow_password, "0", sizeof(o.allow_password))) return(false);
  ...
  if (!GetRegKey("show_script_window", o.show_script_window, "0", sizeof(o.show_script_window))) return(false);
 ...
 * add by mey 20131008
*/
int SetRegistryValue_DWORD(HKEY regkey, const char *name, DWORD data)
{
  /* set a registry string */
  if(RegSetValueEx(regkey, name, 0, REG_DWORD, (const BYTE*)&data, sizeof(DWORD)) != ERROR_SUCCESS)
    {
      /* Error writing registry value */
      ShowLocalizedMsg(GUI_NAME, ERR_WRITE_REGVALUE, GUI_REGKEY_HKCU, name);
      return(0);
...
LONG GetRegistryValue_binary(HKEY regkey, const char *name, char *data, DWORD len)
{
		LONG status;
    //char sz[256]; 
    DWORD dwtype; 
    ...
    status = RegQueryValueEx(regkey, name, NULL, &dwtype, (byte*)data, &data_len); 
 ...
 * add by mey 20131008
*/
int SetRegistryValue_binary(HKEY regkey, const char *name, char* data, int len)
{
  /* set a registry string */
  if(RegSetValueEx(regkey, name, 0, REG_BINARY, (const BYTE*)data, len) != ERROR_SUCCESS)
    {
      /* Error writing registry value */
      ShowLocalizedMsg(GUI_NAME, ERR_WRITE_REGVALUE, GUI_REGKEY_HKCU, name);
      return(0);
